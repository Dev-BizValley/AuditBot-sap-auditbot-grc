...
// Components
import CrossSystemUsersFilter from '../component/auditorcomponent/CrossSystemUsersFilter';
...
const CrossSystemUsersReport = () => {
  const classes = useStyles();
  const dispatch = useDispatch();
  const location = useLocation();

  const token = useSelector(state => state.login?.token);
  const crossSystemUsersReport = useSelector(state => state.auditor?.crossSystemUsersReport) || {};
  ...
  useEffect(() => {
    ...
    dispatch(action.clearCrossSystemUsersReport());
        ...
        <Box className={classes.scrollContainer}>
          <Container maxWidth={false} disableGutters className={classes.contentContainer}>
            <Grid container spacing={3}>
              <Grid item xs={12}>
                {loader && <Loader />}
                {Object.keys(crossSystemUsersReport).length > 0 && crossSystemUsersReport.data && crossSystemUsersReport.data.length > 0 ? (
                  <Fade in={true} timeout={600}>
                    <Paper elevation={0} className={classes.tablePaper}>
                      <MUGRCReportTable
                        colors={colors}
                        header={crossSystemUsersReport.header}
                        data={crossSystemUsersReport.data}
                        name={crossSystemUsersReport.reportName && crossSystemUsersReport.reportName[0] ? crossSystemUsersReport.reportName[0] : "Cross System Users Details"}
                        options={{ tableBodyMaxHeight: 'calc(100vh - 250px)' }}
                      />
                    </Paper>
                  </Fade>
                ) : (
                  !loader && Object.keys(crossSystemUsersReport).length > 0 && (
...
export default CrossSystemUsersReport;
