...
class GRCFilter extends Component {
    ...
    onfilterSumbit = () => {
        ...
        this.props.riskTechGrcReport(this.props.token,
            ...
            this.props.userinput, this.props.account.selectedValue, this.props.userGroup.selectedValue)
    ...
    render() {
        ...
        return (
            <Box position="relative" width="100%" style={{ minHeight: this.state.isVisible ? 'auto' : '0px', backgroundColor: this.state.isVisible ? 'transparent' : '#FFFFFF' }}>
                <Card elevation={0} style={{
                    ...
                    <CardContent style={{ padding: '8px 16px', overflow: 'visible' }}>
                        <Box display="flex" alignItems="center" flexWrap="nowrap" width="100%" gridGap={8}>
                            ...
                            <Box flex="0 0 140px" minWidth={140}>
                                <FilterSingleSelectDropDown values={reportView} preSelected={this.props.reportView.selectedValue} changeEventCallBack={this.changeReportViewType} label="Report View" />
                            </Box>
                            <Box flex="0 0 120px" minWidth={120}>
                            ...
                            <Box flex="0 0 auto" style={{ alignSelf: 'flex-end', marginBottom: '4px' }}>
                                <Button
                                    variant="contained"
                    ...
                    <Grid container spacing={1} style={{ marginTop:'auto', marginBottom:'auto', height:'inherit', minHeight:'inherit'}} >
                        <Grid item md={1/2} style={{ marginTop:'auto', marginBottom:'auto'}}>
...
const mapDispatchToProps = dispatch => { // this methos used for dispatch action to reducer
    return {
        ...
        submitFilter: (token, riskType, sapSystem, client, riskLevel, businessModule, level, breakDown, riskId, reportType, mitigation) => dispatch(action.submitFilter({ data: { token, riskType, sapSystem, client, riskLevel, businessModule, level, breakDown, riskId, reportType, mitigation } })),
        riskTechGrcReport: (token, sapSystem, client, level, riskType, riskLevel, businessModule, riskId, reportView, userinput, account, userGroup) => dispatch(action.riskTechGrcReport({ data: { token, sapSystem, client, level, riskType, riskLevel, businessModule, riskId, reportView, userInput: userinput, account, userGroup } }))
    };
}
